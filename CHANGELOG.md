...
### 2025-xx v2.3.0
- add: Change Base Shape Key機能を追加。指定シェイプキーをBasis形状に適用し、逆シェイプキーを作成
- add: Sort Shape Keys by Name機能を追加。シェイプキーを名前順にソート
- add: Swap Shape Keys機能を追加。2つのシェイプキーの位置を入れ替え
- add: MizoresCustomExporter連携用のベース変更・並び替えAPIを追加

### 2025-xx v2.2.0
- add: %AS:DIRECT%オプション追加。既存シェイプキーを上書きする際の適用方法を選択可能に
  - デフォルト（%AS%）: デルタ加算モード - Basisからの差分を既存シェイプキーに加算
  - %AS:DIRECT%: 直接適用モード - 既存シェイプキーの座標に直接変形を適用
- change: タグオプションの組み合わせをサポート（例: %AS:ALL:DIRECT%, %AS:DIRECT:BASE:xxx%）
- fix: すでに同名シェイプキーが存在する場合に%AS%で形状変形を適用する機能が正しく動作していなかったのを修正
- fix: 左右対称編集が有効なメッシュで、シェイプキーの左右分割系の機能が正常に動作していなかったのを修正
- add: %AS:BASE:基準シェイプキー名%で始まるモディファイアで複合シェイプキー作成機能を追加。指定した基準シェイプキーとの差分のみをシェイプキーとして適用
- add: %AS:ALL:BASE:基準シェイプキー名%で始まるSurfaceDeform/MeshDeformモディファイアで、ターゲットオブジェクトの各シェイプキーを基準シェイプキーとの差分として個別に適用する機能を追加
- add: `シェイプキー名@BASE:ベースシェイプキー名`形式のシェイプキー名を持つシェイプキーに対して、ベースシェイプキーとの差分のみをシェイプキーとして適用する機能を追加
- add: 個別シェイプキー適用時、ターゲットオブジェクトのシェイプキー名に@BASE:基準シェイプキー名を含めることで、シェイプキーごとに異なる基準シェイプキーを指定できる機能を追加


### 2024-11-25 v2.1.0
- add: 以下の条件を全て満たす場合、ターゲットオブジェクトのシェイプキーによる変形結果を個別にシェイプキーとして適用するように
  - SurfaceDeformまたはMeshDeformモディファイア
  - モディファイア名が%AS:ALL%で始まる、またはモディファイア名が%AS%で始まりターゲットオブジェクトの最初のシェイプキーの名前が"All"
  - モディファイアのターゲットオブジェクトにシェイプキーが2つ以上存在している
- add: %AS:I%で始まるアーマチュアモディファイアでは、ボーンの移動と回転とスケールを反転した状態でシェイプキーとして適用されるように
