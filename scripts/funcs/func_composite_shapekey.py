...
def apply_multiple_composite_shapekeys(obj, modifier):
    ...
    for i in range(1, len_key_blocks):
        ...
        if base_shapekey_name:
            # 複合シェイプキーとして処理
            try:
                _apply_composite_shapekey_for_multiple(obj, modifier, base_shapekey_name, clean_name, keep_modifier)
            except ValueError as e:
...
def _apply_composite_shapekey_for_multiple(obj, modifier, base_shapekey_name, target_shapekey_name, keep_modifier):
