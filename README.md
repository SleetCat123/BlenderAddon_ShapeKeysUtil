...
- `%S%`左右分割後のシェイプキーを分割前シェイプキーのすぐ下に移動する
    （処理に時間がかかるため、必要な場合だけ付けることをおすすめします）
...
――  
※ 上記`%LR%`などの制御用文字列は処理結果のシェイプキー名からは削除されます。  

※ 上記の文字列は以下のAssign Tagコマンドで簡単に着脱できます。  

### Assign Tag
現在のシェイプキーに対し、(Tag) Separate All Shape Key Left and Right の処理用テキストをつけたりはずしたりします。
...
　　注意：Enableにチェックが入っていないと操作できません
　　注意：多くのシェイプキーで有効にするとシェイプキー移動処理に時間がかかります
